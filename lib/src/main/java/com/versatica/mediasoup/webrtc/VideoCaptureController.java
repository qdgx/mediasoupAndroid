...
//    }
    ...
    public VideoCaptureController(CameraEnumerator cameraEnumerator,
        ...
        if (constraints.containsKey("mandatory")
                && HashMap.class.isInstance(constraints.get("mandatory"))) {
            videoConstraintsMandatory = (HashMap) constraints.get("mandatory");
        }
        ...
        if (videoConstraintsMandatory != null) {
            width = videoConstraintsMandatory.containsKey("minWidth")
                    ? (int) videoConstraintsMandatory.get("minWidth")
                    : DEFAULT_WIDTH;
            height = videoConstraintsMandatory.containsKey("minHeight")
                    ? (int) videoConstraintsMandatory.get("minHeight")
                    : DEFAULT_HEIGHT;
            fps = videoConstraintsMandatory.containsKey("minFrameRate")
                    ? (int) videoConstraintsMandatory.get("minFrameRate")
                    : DEFAULT_FPS;
        }
    }
...
//    }
    ...
    private String getFacingMode(HashMap mediaConstraints) {
        return
            mediaConstraints == null
                ? null
                : (String) mediaConstraints.get("facingMode");
    }
...
//    }
    ...
    private String getSourceIdConstraint(HashMap mediaConstraints) {
        if (mediaConstraints != null
                ...
                && ArrayList.class.isInstance(mediaConstraints.get("optional"))) {
            ArrayList optional = (ArrayList) mediaConstraints.get("optional");

            for (int i = 0, size = optional.size(); i < size; i++) {
                if (HashMap.class.isInstance(optional.get(i))) {
                    HashMap option = (HashMap) optional.get(i);

                    if (option.containsKey("sourceId")
                            && String.class.isInstance(option.get("sourceId"))) {
                        return (String) option.get("sourceId");
                    }
                }
            }
        }
